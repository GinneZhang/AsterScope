...
def test_run_ragflow_recursive_retrieval_accumulates_new_hits():
    ...
    agent._generate_ragflow_next_step_queries = lambda question, current_query, missing_information, knowledge_blocks, limit=2: [
        {
            "question": "What does GmbH stand for?",
            "query": "Gesellschaft mit beschrankter Haftung",
        }
    ]

    def _collect_candidate_pool(query, top_k=5, additional_queries=None, include_follow_ups=False):
        return [
            {
                "doc_id": "gmbh",
                "chunk_index": 0,
